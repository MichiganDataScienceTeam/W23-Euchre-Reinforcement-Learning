    ...
    for s in non_terminal_states:
        ...
        for i in range(num_actions):
            ...
            for s_prime in range(num_states):
                reward_from_action = rewards_model.loc[s, a]
                transition_prob = transition_matrix[s_prime].loc[(s,a)]
                value += transition_prob * (reward_from_action + gamma * value_map[s_prime])
            action_vals[i] = value
        # save that action in the new_policy
        new_action = actions[np.argmax(action_vals)]
        new_policy.loc[s,new_action] = 1
...
if __name__=="__main__":
    ...
    while True:
        ...
        value_map = policy_evaluation(value_map, policy, rewards_model, transition_matrix, theta)
        policy, stable_policy = policy_improvement(value_map, policy, rewards_model, transition_matrix)
